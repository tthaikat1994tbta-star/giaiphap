...
# ĐÂY PHẢI LÀ LỆNH STREAMLIT ĐẦU TIÊN
st.set_page_config(
...
# Sau khi set_page_config mới được dùng các lệnh st khác
...
def load_data():
    ...
    # Khởi tạo 20 bàn mặc định nếu chưa có file backup
...
for i, (item, price) in enumerate(config.menu.items()):
    with cols[i % 3]:
        # Lấy giá trị hiện tại của món ăn từ session_state
        current_val = st.session_state.orders[so_ban].get(item, 0)
        ...
        # Nếu người dùng thay đổi số lượng, cập nhật và lưu ngay
        if qty != current_val:
...
if total_price > 0:
    ...
    with col_btn:
        if st.button("🔥 THANH TOÁN & BÁO CÁO", type="primary", use_container_width=True):
            ...
            # Reset bàn về 0 sau khi thanh toán
